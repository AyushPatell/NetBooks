<?php
	...
	function generateRow($from, $to, $conn){
		...
		$stmt = $conn->prepare("SELECT *, order_id AS orderid FROM order_master  JOIN user_master ON user_id=user_id WHERE order_date BETWEEN '$from' AND '$to' ORDER BY order_date DESC");
		$stmt->execute();
		$total = 0;
		foreach($stmt as $row){
			$stmt = $conn->prepare("SELECT * FROM order_master LEFT JOIN book_master ON book_id=book_id WHERE order_id=:id");
			$stmt->execute(['id'=>$row['salesid']]);
			$amount = 0;
			foreach($stmt as $details){
				$subtotal = $details['order_price']*$details['order_quantity'];
				$amount += $subtotal;
			}
			$total += $amount;
			$contents .= '
